...
class _CategoriesViewState extends State<CategoriesView>
                    ...
                    child: Opacity(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [BoxShadow(color: _toastColor.withOpacity(0.4), blurRadius: 12, offset: const Offset(0, 4))],
                        ),
                        child: Row(children: [
                          Icon(_toastIcon, color: Colors.white, size: 20),
                          const SizedBox(width: 10),
                          Expanded(child: Text(_toastMsg, style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600, height: 1.4))),
                          GestureDetector(
  ...
  Widget _buildHeader(bool isDark) {
    return Row(children: [
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Lịch sử', style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
        Text('Tất cả giao dịch của bạn', style: TextStyle(fontSize: 13, color: Colors.grey[500])),
      ])),
      GestureDetector(
  ...
  Widget _buildFilterRow(bool isDark) {
    return Row(children: [
      ...
      const Spacer(),
      GestureDetector(
        onTap: () => setState(() => _sortBy = _sortBy == 'date' ? 'amount' : 'date'),
        child: Container(
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(_sortBy == 'date' ? 'Ngày' : 'Số tiền',
                style: TextStyle(fontSize: 11, color: Theme.of(context).brightness == Brightness.dark ? Colors.grey[400] : Colors.grey[600])),
          ]),
        ),
      ),
    ]);
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: active ? c : (isDark ? Colors.grey[700]! : Colors.grey[200]!), width: active ? 1.5 : 1),
          boxShadow: active ? [BoxShadow(color: c.withOpacity(0.3), blurRadius: 6, offset: const Offset(0, 2))] : [],
        ),
  ...
  Widget _buildSummaryBar(bool isDark) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors: [_teal, Color(0xFF0097A7)], begin: Alignment.centerLeft, end: Alignment.centerRight),
        borderRadius: BorderRadius.circular(14),
        boxShadow: [BoxShadow(color: _teal.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 3))],
      ),
      child: Row(children: [
        Expanded(child: FittedBox(fit: BoxFit.scaleDown, child: _summaryCol('↑ Thu nhập', _fmt(_totalIncome) + 'đ', Colors.greenAccent[200]!))),
        Container(width: 1, height: 36, color: Colors.white.withOpacity(0.3)),
        Expanded(child: FittedBox(fit: BoxFit.scaleDown, child: _summaryCol('↓ Chi tiêu', _fmt(_totalExpense) + 'đ', Colors.red[200]!))),
        Container(width: 1, height: 36, color: Colors.white.withOpacity(0.3)),
        Expanded(child: FittedBox(fit: BoxFit.scaleDown, child: _summaryCol('📋 Giao dịch', '$count', Colors.white))),
      ]),
    );
  }
  ...
  Widget _buildTxList(bool isDark) {
    ...
    if (grouped.isEmpty) {
      return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        Text('Không có giao dịch nào', style: TextStyle(fontSize: 16, color: Colors.grey[500])),
        const SizedBox(height: 8),
        Text(_searchQuery.isNotEmpty ? 'Thử tìm kiếm với từ khóa khác' : 'Thêm giao dịch để xem lịch sử',
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (_, i) {
          ...
          final dayInc  = txs.where((t) => t['isIncome'] as bool).fold(0.0, (s, t) => s + (t['amount'] as double));
          final dayExp  = txs.where((t) => !(t['isIncome'] as bool)).fold(0.0, (s, t) => s + (t['amount'] as double));

          return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Padding(
              padding: const EdgeInsets.symmetric(vertical: 12),
              child: Row(children: [
                Text(dateKey, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: isDark ? Colors.grey[300] : Colors.grey[700])),
                const Spacer(),
                if (dayInc > 0) Text('+${_fmt(dayInc)}đ', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.green[600])),
                if (dayExp > 0) ...[
                  if (dayInc > 0) const SizedBox(width: 8),
                  Text('-${_fmt(dayExp)}đ', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.red[500])),
                ],
              ]),
            ),
            Container(
              ...
              child: Column(children: txs.asMap().entries.map((e) {
                return _buildTxItem(e.value, isDark, isLast: e.key == txs.length - 1);
              }).toList()),
            ),
            const SizedBox(height: 4),
          ]);
  ...
  Widget _buildTxItem(Map<String, dynamic> tx, bool isDark, {bool isLast = false}) {
      ...
      Dismissible(
        ...
        confirmDismiss: (_) async {
          return await showDialog<bool>(
            ...
            builder: (_) => AlertDialog(
              ...
              title: const Text('Xóa giao dịch?', style: TextStyle(fontSize: 16)),
              content: Text('Xóa "$title" - ${_fmt(amount)}đ?', style: TextStyle(fontSize: 13, color: Colors.grey[600])),
              actions: [
                TextButton(onPressed: () => Navigator.pop(context, false), child: Text('Hủy', style: TextStyle(color: Colors.grey[500]))),
                ElevatedButton(
                  onPressed: () => Navigator.pop(context, true),
                  style: ElevatedButton.styleFrom(backgroundColor: Colors.red[500], elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
        ...
        child: Padding(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(title, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: isDark ? Colors.white : const Color(0xFF1A1A1A)), maxLines: 1, overflow: TextOverflow.ellipsis),
              ...
              Row(children: [
                ...
                const SizedBox(width: 6),
                Text(_fmtDate(date), style: TextStyle(fontSize: 10, color: Colors.grey[500])),
              ]),
            ])),
            Text('${isInc ? '+' : '-'}${_fmt(amount)}đ', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: color)),
          ]),
        ),
      ),
      if (!isLast) Divider(height: 1, indent: 72, color: isDark ? Colors.grey[700] : Colors.grey[100]),
    ]);
  }
            ...
            GridView.count(
              ...
              children: List.generate(12, (i) {
                ...
                return GestureDetector(
                  onTap: () {
                    Navigator.pop(ctx);
                    setState(() { _filterMonth = m; _filterYear = selYear; });
                    _loadTransactions();
                  },
                  child: Container(
                    decoration: BoxDecoration(
                      color: isSelM ? _teal : (isDark ? const Color(0xFF3A3A3A) : Colors.grey[100]),
                      borderRadius: BorderRadius.circular(10),
                    ),
                    child: Center(child: Text('T$m', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: isSelM ? Colors.white : (isDark ? Colors.grey[300] : Colors.grey[700])))),
                  ),
                );
              }),
            ),
  ...
  Widget _buildRuleTab(bool isDark) {
    ...
    const wantsCats = ['Giải trí & xã hội','Mua sắm','Mua sắm cá nhân','Du lịch','Ăn ngoài','Cafe','Thú vui'];
    ...
    if (income == 0) {
      return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const SizedBox(height: 16),
        Text('Chưa có thu nhập tháng này', style: TextStyle(fontSize: 16, color: Colors.grey[500])),
        const SizedBox(height: 8),
        Text('Thêm giao dịch thu nhập để xem phân tích', style: TextStyle(fontSize: 13, color: Colors.grey[400])),
      ]));
    }
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
        Container(
          ...
          child: Column(children: [
            const Row(children: [Text('🎯', style: TextStyle(fontSize: 20)), SizedBox(width: 8), Expanded(child: Text('Quy tắc 50/30/20', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)))]),
            const SizedBox(height: 4),
        ...
        const SizedBox(height: 16),
        _ruleGroup(isDark, emoji: '🏠', label: '50% — Thiết yếu', budget: needs, spent: needsSpent, color: Colors.green[600]!, detail: needsDetail, tip: 'Ăn uống, nhà ở, đi lại, tiện ích, sức khỏe'),
        const SizedBox(height: 12),
        _ruleGroup(isDark, emoji: '🎬', label: '30% — Linh hoạt', budget: wants, spent: wantsSpent, color: Colors.amber[700]!, detail: wantsDetail, tip: 'Giải trí, mua sắm, ăn ngoài, du lịch'),
        const SizedBox(height: 12),
        _ruleGroup(isDark, emoji: '💰', label: '20% — Tiết kiệm', budget: savings, spent: savingsSpent, color: Colors.blue[600]!, detail: savingsDetail, tip: 'Tiết kiệm, đầu tư, quỹ dự phòng'),
        const SizedBox(height: 12),
        _buildRuleInsight(isDark, needsSpent, wantsSpent, savingsSpent, needs, wants, savings, income),
      ]),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ClipRRect(borderRadius: BorderRadius.circular(6),
            child: LinearProgressIndicator(value: progress.clamp(0.0, 1.0), minHeight: 8, backgroundColor: isDark ? Colors.grey[700] : Colors.grey[100], valueColor: AlwaysStoppedAnimation(isOver ? Colors.red : color))),
          const SizedBox(height: 8),
          Row(children: [
  ...
  Widget _buildRuleInsight(bool isDark, double needsS, double wantsS, double savingsS, double needsB, double wantsB, double savingsB, double income) {
    final messages = <Map<String, dynamic>>[];
    if (needsS > needsB) messages.add({'icon': '⚠️', 'color': Colors.red[500]!, 'text': 'Chi thiết yếu vượt 50% — xem lại nhà ở và ăn uống'});
    else messages.add({'icon': '✅', 'color': Colors.green[600]!, 'text': 'Chi thiết yếu hợp lý — đang kiểm soát tốt'});
    if (wantsS > wantsB) messages.add({'icon': '🎯', 'color': Colors.amber[700]!, 'text': 'Giải trí/mua sắm vượt 30% — cân nhắc cắt giảm'});
    if (savingsS >= savingsB) messages.add({'icon': '💪', 'color': Colors.blue[600]!, 'text': 'Tiết kiệm đạt mục tiêu 20% — xuất sắc!'});
    else messages.add({'icon': '💡', 'color': Colors.blue[400]!, 'text': 'Cần tiết kiệm thêm ${_fmt(savingsB - savingsS)}đ để đạt 20%'});

    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: isDark ? const Color(0xFF2C2C2C) : Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: isDark ? Colors.grey[700]! : Colors.grey[200]!)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [const Text('🧠', style: TextStyle(fontSize: 20)), const SizedBox(width: 8), Text('Nhận xét tháng này', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: isDark ? Colors.white : Colors.black87))]),
        const SizedBox(height: 12),
        ...messages.map((m) => Padding(padding: const EdgeInsets.only(bottom: 8),
  ...
  Widget _buildZeroBasedTab(bool isDark) {
    ...
    if (income == 0) {
      return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        Text('Chưa có thu nhập tháng này', style: TextStyle(fontSize: 16, color: Colors.grey[500])),
        const SizedBox(height: 8),
        Text('Thêm thu nhập để bắt đầu phân bổ ngân sách', style: TextStyle(fontSize: 13, color: Colors.grey[400])),
      ]));
    }
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
        Container(
          padding: const EdgeInsets.all(18),
          decoration: BoxDecoration(
            gradient: LinearGradient(colors: isBalanced ? [Colors.green[600]!, Colors.green[800]!] : unallocated > 0 ? [const Color(0xFF8B5CF6), const Color(0xFF6D28D9)] : [Colors.red[600]!, Colors.red[800]!], begin: Alignment.topLeft, end: Alignment.bottomRight),
            borderRadius: BorderRadius.circular(18),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Container(
                ...
                child: Text(isBalanced ? '✅ Cân bằng!' : unallocated > 0 ? '⚡ Còn dư' : '⚠️ Vượt ngân sách', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w600)),
              ),
            ]),
            const SizedBox(height: 4),
            ...
            IntrinsicHeight(child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, crossAxisAlignment: CrossAxisAlignment.center, children: [
              ...
              Expanded(child: _zeroStatCol('Đã chi', '${_fmt(totalAllocated)}đ', Colors.greenAccent[200]!)),
              Container(width: 1, color: Colors.white24),
              Expanded(child: _zeroStatCol(unallocated >= 0 ? 'Còn lại' : 'Vượt quá', '${_fmt(unallocated.abs())}đ', unallocated >= 0 ? Colors.amber[200]! : Colors.red[200]!)),
            ])),
            const SizedBox(height: 14),
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Text('Tỷ lệ đã phân bổ', style: TextStyle(color: Colors.white70, fontSize: 11)),
              Text(income > 0 ? '${(totalAllocated / income * 100).toStringAsFixed(1)}%' : '0%', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.bold)),
            ]),
            const SizedBox(height: 6),
            ClipRRect(borderRadius: BorderRadius.circular(6),
              child: LinearProgressIndicator(value: income > 0 ? (totalAllocated / income).clamp(0.0, 1.0) : 0, minHeight: 8, backgroundColor: Colors.white24, valueColor: AlwaysStoppedAnimation(isBalanced ? Colors.greenAccent : Colors.white))),
          ]),
        ),
        const SizedBox(height: 12),
        Container(
          padding: const EdgeInsets.all(14),
          decoration: BoxDecoration(color: isDark ? const Color(0xFF2C2C2C) : const Color(0xFFF0FAFA), borderRadius: BorderRadius.circular(14), border: Border.all(color: _teal.withOpacity(0.2))),
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('💡', style: TextStyle(fontSize: 18)),
            const SizedBox(width: 10),
            Expanded(child: Text('Zero-Based Budgeting: Thu nhập − Tất cả chi tiêu = 0. Mỗi đồng được phân công nhiệm vụ cụ thể, không để tiền "thất lạc".', style: TextStyle(fontSize: 12, height: 1.5, color: isDark ? Colors.grey[300] : const Color(0xFF2C7873)))),
          ]),
        ),
        const SizedBox(height: 12),
        Container(
            ...
            Padding(
              ...
              child: Row(children: [
                Container(width: 36, height: 36,
                  decoration: BoxDecoration(color: unallocated >= 0 ? Colors.green.withOpacity(0.12) : Colors.red.withOpacity(0.12), borderRadius: BorderRadius.circular(10)),
                  child: Icon(unallocated >= 0 ? Icons.savings_rounded : Icons.warning_rounded, color: unallocated >= 0 ? Colors.green : Colors.red, size: 18)),
                const SizedBox(width: 10),
                Expanded(child: Text(unallocated >= 0 ? 'Chưa phân bổ / Tiết kiệm' : 'Vượt ngân sách', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: unallocated >= 0 ? Colors.green : Colors.red))),
                Text('${unallocated >= 0 ? '' : '-'}${_fmt(unallocated.abs())}đ', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: unallocated >= 0 ? Colors.green : Colors.red)),
              ]),
            ),
          ]),
        ),
  ...
  Widget _buildZeroInsight(bool isDark, double income, double allocated, double unallocated) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: isDark ? const Color(0xFF2C2C2C) : Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: isDark ? Colors.grey[700]! : Colors.grey[200]!)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [const Text('🧠', style: TextStyle(fontSize: 18)), const SizedBox(width: 8), Text('Gợi ý cải thiện', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: isDark ? Colors.white : Colors.black87))]),
        const SizedBox(height: 12),
    ...
    return sorted.asMap().entries.map((entry) {
      ...
      return Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(16, 10, 16, 10), child: Column(children: [
          Row(children: [
            Container(width: 36, height: 36, decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(10)), child: Icon(_catIcon(cat), color: color, size: 18)),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ClipRRect(borderRadius: BorderRadius.circular(4),
            child: LinearProgressIndicator(value: (pct / 100).clamp(0.0, 1.0), minHeight: 5, backgroundColor: isDark ? Colors.grey[700] : Colors.grey[100], valueColor: AlwaysStoppedAnimation(color))),
        ])),
  ...
  // ── Bottom nav — Home | History | Voice | Plan | Profile ──
  ...
  Widget _navItem(IconData icon, bool isActive, {VoidCallback? onTap, String label = ''}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    ...
    return GestureDetector(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(
          ...
          decoration: BoxDecoration(color: isActive ? teal.withOpacity(0.12) : Colors.transparent, borderRadius: BorderRadius.circular(12)),
