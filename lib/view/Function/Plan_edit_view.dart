...
class PlanEditView extends StatefulWidget {
  /// Danh sách rows từ plan['expense_table']
  final List<Map<String, dynamic>> rows;

  /// Các giá trị đã chỉnh trước đó (nếu mở edit lần 2)
...
class _PlanEditViewState extends State<PlanEditView> {
  ...
  // controller cho từng category
  final Map<String, TextEditingController> _ctrls = {};

  // ── THÊM MỚI: các khoản chi người dùng tự thêm ───────
  final List<Map<String, dynamic>> _extraRows = [];
  final Map<String, TextEditingController> _extraCtrls = {};
  // ─────────────────────────────────────────────────────

  // controller thu nhập tổng
  late TextEditingController _incomeCtrl;
  bool _showIncomeField = false; // toggle hiện/ẩn ô nhập lương

  // màu cho từng dòng (giống AnalysisView)
  ...
  void initState() {
    super.initState();
    // Lấy thu nhập gốc từ initialEdits hoặc tính từ tổng rows
    final initIncome = widget.initialEdits['__income__']
        ?? widget.rows.fold<int>(0, (s, r) =>
  ...
  // Tổng tất cả số chi đang nhập (cả khoản gốc lẫn tự thêm)
  int get _total {
  ...
  void _save() {
    ...
    // Khoản tự thêm — dùng prefix để phân biệt
  ...
  void _reset() {
    setState(() {
      final origIncome = widget.rows.fold<int>(0, (s, r) =>
          s + ((r['amount'] as num?)?.toInt() ?? 0));
      _incomeCtrl.text = origIncome.toString();
      for (final row in widget.rows) {
        final cat     = row['category'] as String? ?? '';
        final origAmt = (row['amount'] as num?)?.toInt() ?? 0;
        _ctrls[cat]?.text = origAmt.toString();
      }
      // Xóa khoản tự thêm khi reset
      for (final c in _extraCtrls.values) c.dispose();
      _extraCtrls.clear();
      _extraRows.clear();
    });
            ...
            child: Column(mainAxisSize: MainAxisSize.min,
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  filled: true,
                  fillColor: isDark
                      ? const Color(0xFF3A3A3A) : Colors.grey[50],
                  ...
                  onPressed: () {
                    if (formKey.currentState!.validate()) {
                      ...
                      setState(() {
                        ...
                        _extraCtrls[name] =
                            TextEditingController(text: amt.toString());
                      });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        _buildHeader(isDark),
        _buildIncomeSection(isDark),   // ← MỚI
        ...
        return Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: Row(children: [
                  ...
                  const Expanded(
                    child: Text('Thu nhập / tháng của bạn',
            ...
            AnimatedCrossFade(
              ...
              crossFadeState: _showIncomeField
  ...
  // ── Thanh tổng chi + so sánh với thu nhập ────────────
      ...
      builder: (_, __) {
        ...
        return Container(
          ...
          child: Column(children: [
            Row(children: [
              Icon(
                isOver ? Icons.warning_amber_rounded
                    : isOk ? Icons.check_circle_rounded
            ...
            // Dòng so sánh — chỉ hiện khi có thu nhập
            if (hasIncome) ...[
              ...
              Container(
                ...
                child: Row(children: [
                  Expanded(child: _miniStat(
                      'Thu nhập', '${_fmt(income)} đ', Colors.white)),
                  Container(width: 1, height: 32,
                      color: Colors.white.withOpacity(0.3)),
                  Expanded(child: _miniStat(
                      'Chi tiêu', '${_fmt(total)} đ', Colors.white)),
                  Container(width: 1, height: 32,
                      color: Colors.white.withOpacity(0.3)),
                  Expanded(child: _miniStat(
            ...
            Expanded(
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  filled: true,
                  fillColor: isDark
                      ? const Color(0xFF3A3A3A) : color.withOpacity(0.05),
                  border: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(10),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              suffixText: 'đ',
              suffixStyle: const TextStyle(
                  color: _orange, fontWeight: FontWeight.bold, fontSize: 16),
              filled: true,
              fillColor: isDark
                  ? const Color(0xFF3A3A3A) : _orange.withOpacity(0.04),
              border: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(10),
